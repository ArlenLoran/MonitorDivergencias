...
function getContext() {
  ...
  if (!ctx) {
    // If not in SP, we might want a mock for local dev, 
    // but the instruction says "deve rodar dentro de uma página SharePoint"
    throw new Error(
...
export async function spDeleteList(listName: string): Promise<SpResult<true>> {
  const ln = sanitizeInput(listName);
  try {
    const url = `${spSiteUrl()}/_api/web/lists/getByTitle('${escapeODataString(ln)}')`;
    const resp = await fetchWithDigestRetry(url, {
      method: 'POST',
      headers: {
        'Accept': 'application/json; odata=verbose',
